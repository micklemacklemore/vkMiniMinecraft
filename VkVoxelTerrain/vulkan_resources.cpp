...
void generateMipmaps(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool, VkQueue queue, VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
    ...
    for (uint32_t i = 1; i < mipLevels; i++) {
        ...
        // change layout of previous mipLevel to TRANSFER SRC OPTIMAL (we assume that the image layout is already 
        // TRANSFER_DST_OPTIMAL, so we don't need to change this before hand)
        ...
        // change previous mip level to SHADER READ ONLY
    ...
    // change the very last mip level to SHADER READ ONLY
